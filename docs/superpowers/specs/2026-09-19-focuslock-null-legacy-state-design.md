...
插件目录新增默认兼容状态模板，并由安装器随 worker 一起部署。worker 增加兼容状态构造与安全读取函数。文件缺失、空白、JSON 损坏或根对象为 `null` 时，优先从模板重建一个可写的安全状态对象；随后沿用既有原子写入流程。修复现场文件前保留原始损坏副本，再部署 worker 并触发 SYSTEM 同步。
...
- FocusLock 的可迁移源码位于仓库 `focus-lock` 目录，ProgramData 只保留部署副本和状态。
