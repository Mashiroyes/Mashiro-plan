...
- [ ] **Step 1: Add assertions for loopback-only HTTP fallback and proxy bypass.**
- [ ] **Step 2: Run `pwsh -NoProfile -File plugin/mashirobot-plugin-game-timer/block/tests/test-worker.ps1` and require the new assertions to fail.**
- [ ] **Step 3: Implement `HttpClient` PUT to `http://127.0.0.1:9097/configs?force=true` with `UseProxy=false`, bearer authentication, bounded timeout, and 200/204 validation.**
- [ ] **Step 4: Retry HTTP and named-pipe transports within the existing 12-second budget and preserve both transport errors.**
- [ ] **Step 5: Run the focused worker test and `node --test plugin/mashirobot-plugin-game-timer/block/tests/*.test.mjs`; require all tests to pass.**
...
- [ ] **Step 1: Set `enable_external_controller: true` and bind generated controller entries to `127.0.0.1:9097`.**
- [ ] **Step 2: Apply the current generated configuration through the user-accessible pipe and verify authenticated `/version` on port 9097.**
- [ ] **Step 3: Deploy the source worker to the installed worker path using elevation and verify hashes match.**
- [ ] **Step 4: Trigger SYSTEM reapply with pending reload state; require fresh success, task result 0, and pending false.**
- [ ] **Step 5: Verify Bilibili release/access, QQ retention, and loopback-only listening.**
- [ ] **Step 6: Commit and push the tested source changes to `Mashiroyes/Mashiro-plan` main.**
