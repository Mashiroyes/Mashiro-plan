...
完整的环境、依赖、数据库迁移、安装顺序和验收命令见 [新电脑部署指南](docs/new-computer-setup.md)。

迁移时务必注意：

- `sqlite\openclaw-planner.sqlite` 被 Git 忽略，必须从旧电脑单独迁移。
- 新电脑不迁移 LibreHardwareMonitor、PawnIO 或温度监控计划任务。
- OpenClaw/微信凭据、ProgramData 运行状态和本机软件路径需要单独恢复或重新配置。
## 架构
